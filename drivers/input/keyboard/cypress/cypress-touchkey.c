...
static int bln_bln_breathing_step_count = 0;
...
static void bln_breathe_process(struct work_struct *work)
{
	...
	if (bln_breathing_steps[bln_breathing_step_idx].start <= bln_breathing_steps[bln_breathing_step_idx].end) {
		...
		if (data > bln_breathing_steps[bln_breathing_step_idx].end) {
			...
			if (bln_breathing_step_idx >= bln_bln_breathing_step_count) bln_breathing_step_idx = 0;
			data = bln_breathing_steps[bln_breathing_step_idx].start;
		}
	} else {
		data = bln_breathing_steps[bln_breathing_step_idx].start -
...
void bln_reset_bln_breathing_steps(void)
{
	...
	bln_bln_breathing_step_count = 0;
...
{
	int count = (bln_bln_breathing_step_count == 0 ? 1 : bln_bln_breathing_step_count);
	int i, len = 0;
	for (i = 0; i<count; i++) {
...
static ssize_t bln_bln_breathing_steps_write( struct device *dev, struct device_attribute *attr, const char *buf, size_t size )
{
	int ret;
	...
	if (bln_bln_breathing_step_count >= MAX_BLN_BREATHING_STEPS) return -EINVAL;
	ret = sscanf(buf, "%d %d %d %d", &bstart, &bend, &bperiod, &bstep);
	printk(KERN_DEBUG "[TouchKey-BLN] %s: %s\n", __func__, buf);

	if (ret != 4) return -EINVAL;
	bln_breathing_steps[bln_bln_breathing_step_count].start = bstart;
	bln_breathing_steps[bln_bln_breathing_step_count].end = bend;
	bln_breathing_steps[bln_bln_breathing_step_count].period = bperiod;
	bln_breathing_steps[bln_bln_breathing_step_count].step = bstep;
	bln_bln_breathing_step_count++;
	bln_breathing_idx = 0;
...
static DEVICE_ATTR(breathing_steps, S_IRUGO | S_IWUGO, bln_breathing_steps_read, bln_bln_breathing_steps_write);
#endif
